        ...
        try {
            ...
            switch (action) {
                ...
                case "start":
                    ...
                    // Blocca start se la partita è già in corso
                    if (game.isInProgress()) {
                        sendResponse(exchange, 409, "{\"error\":\"Partita già in corso\"}", "application/json");
                        return;
                    }
                ...
                case "abandon":
                    // POST da fetch normale oppure GET da sendBeacon (che non supporta POST con body)
                    // Accettiamo entrambi i metodi per compatibilità con beforeunload
                    state = game.abandonGame();
                    System.out.println("[SERVER] Partita abbandonata dal client. Vince il dealer.");
                    break;
