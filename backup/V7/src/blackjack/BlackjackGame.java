...
public class BlackjackGame {
    ...
    private GameState buildState(boolean revealDealer) {
        ...
        if (revealDealer) {
            ...
            state.setDealerValueLabel(String.valueOf(dealerHand.getValue()));
        } else {
            if (dealerHand.getCards().size() >= 2) {
                Card visibleCard = dealerHand.getCards().get(1);

                state.setDealerCards(List.of(
                        "Carta coperta",
                        visibleCard.getDisplayName()
                ));

                state.setDealerCardImages(List.of(
                        "/assets/back_side.png",
                        visibleCard.getImagePath()
                ));

                String visibleValueLabel;
                
                if (visibleCard.isAce()) {
                    visibleValueLabel = "1/11 + ?";
                } else {
                    visibleValueLabel = visibleCard.getValue() + " + ?";
                }

                state.setDealerValue(0);
                state.setDealerValueLabel(visibleValueLabel);
        ...
        //System.out.println("dealerValueLabel backend = " + state.getDealerValueLabel());
